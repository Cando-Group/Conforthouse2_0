              ...
              <table class="table table-sm table-nowrap card-table">
                ...
                <tbody class="list">
                    ...
                    <?php
                        ...
                        $infoAnnonces = $database->prepare('SELECT * FROM annonces ORDER BY id DESC');
                        $infoAnnonces->execute();

                        while ($dataAnnoncesInfo = $infoAnnonces->fetch()){
                            ?>
                            <tr>
                
                                <td class="orders-order">
                                <a href="../upload/location/<?= $dataAnnoncesInfo['photo']?>"><img src="../upload/location/<?= $dataAnnoncesInfo['photo']?>" alt="" class="img-responsive chambre-img"></a>
                                </td>

                                <?php
                                    /*$photoRecup = $database->prepare('SELECT * FROM demarcheurs WHERE email=:email');
                                    $photoRecup->bindvalue(':email', $dataAnnoncesInfo['email']);
                                    $photoRecup->execute();         

                                    $recuphoto = $photoRecup->fetch();*/
                                ?>
                                <td class="orders-order">
                                <?= $dataAnnoncesInfo['prenoms']?>

                                </td>
                                
                                <td class="orders-product">
                                <?= $dataAnnoncesInfo['tel']?>
                                </td>
                                <td class="orders-date">

                                <!-- Time -->
                                <?= $dataAnnoncesInfo['prix']?> FCFA

                                </td>
                                <td class="orders-total">
                                <?= $dataAnnoncesInfo['localisation']?>
                                </td>
                                <td class="orders-total">
                                <?= $dataAnnoncesInfo['categorie']?>
                                </td>
                                <td class="orders-total">
                                <?= $dataAnnoncesInfo['dates']?>
                                </td>
                                <td class="orders-status">

                                <!-- Badge CANDIDE TU ES SUR LA PARTIE OU IL EST ACTIF OU PAS. GOOD LUCK -->
                                <div class="">
                                    <?php
                                        if ($dataAnnoncesInfo['louer'] == 'non'){
                                            ?>
                                            <span class="badge bg-success-soft" style="font-size:14px;">Non Louer</span>
                                    <?php        
                                        }else{
                                            ?>
                                            <span class="badge bg-danger-soft" style="font-size:14px;">Louer</span>
                                    <?php        
                                        }
                                    ?>
                                </div>
                                ...
                                <td>
                                    <a href="delete_pub.php?id=<?= $dataAnnoncesInfo['id']?>" class="btn btn-danger mb-2">Supprimer</a>
                                </td>
                            </tr>
                    <?php
                        }
