<?php
    ...
    $reqAgentFamiliales = $database->prepare("SELECT * FROM demarcheurs WHERE email=:email");
    $reqAgentFamiliales->bindvalue(":email", $email);
    $reqAgentFamiliales->execute();

    $dataAgentFamiliales = $reqAgentFamiliales->fetch();
...
<div class="container">
<h2 class="breadcrumb-title">Chambres Familiales (<?=$dataAgentFamiliales['nom']. " ". $dataAgentFamiliales['prenoms']?>)</h2>
<ul class="breadcrumb-menu">
<li><a href="index.php">Accueil</a></li>
<li class="active">Chambres Familiales (<?=$dataAgentFamiliales['nom']. " ". $dataAgentFamiliales['prenoms']?>)</li>
</ul>
</div>
</div>
...
$reqEtudiantes = $database->prepare("SELECT * FROM annonces WHERE categorie=:categorie AND louer = :louer AND email=:email ORDER BY id DESC LIMIT :offset, :elementsParPage ");
$reqEtudiantes->bindValue(":categorie", "Chambres Familiale");
        ...
        <div class="listing-bottom">
          <div class="listing-author">
            <div class="listing-author-img">
              <img src="upload/profile/<?=$dataAgentFamiliales['photo']?>" alt="Profile" style="width:45px;height:45px;">
            </div>
...
<!-- Le reste de votre code HTML -->
        ...
        <div class="pagination-area mb-5">
            <div aria-label="Page navigation example">
                <ul class="pagination">
                <li class="page-item">
                    <a class="page-link" href="agentFamiliales.php?email=<?=$email?>&page=<?php echo $pageActuelle - 1; ?>" aria-label="Previous">
                    <span aria-hidden="true"><i class="far fa-angle-double-left"></i></span>
                    </a>
                </li>
                <!-- Vous pouvez laisser les liens statiques pour les pages 1, 2 et 3 pour l'instant -->
                <li class="page-item <?php echo ($pageActuelle == 1) ? 'active' : ''; ?>"><a class="page-link" href="agentFamiliales.php?email=<?=$email?>&page=1">1</a></li>
                <li class="page-item <?php echo ($pageActuelle == 2) ? 'active' : ''; ?>"><a class="page-link" href="agentFamiliales.php?email=<?=$email?>&page=2">2</a></li>
                <li class="page-item <?php echo ($pageActuelle == 3) ? 'active' : ''; ?>"><a class="page-link" href="agentFamiliales.php?email=<?=$email?>&page=3">3</a></li>
                <li class="page-item">
                    <a class="page-link" href="agentFamiliales.php?email=<?=$email?>&page=<?php echo $pageActuelle + 1; ?>" aria-label="Next">
