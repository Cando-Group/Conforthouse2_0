<?php
    ...
    if (isset($_POST['info'])){
        ...
        if (!empty($_POST['nom']) && !empty($_POST['prenoms']) && !empty($_POST['whatsapp'] && !empty($_POST['telephone'])) && !empty($_POST['description'])){
            ...
            $updateInfo = $database->prepare("UPDATE demarcheurs SET nom=:nom, prenoms=:prenoms, telephone=:telephone, whatsapp=:whatsapp, description=:description WHERE email=:email");
            $updateInfo->bindvalue(":nom", $_POST['nom']);
            $updateInfo->bindvalue(":prenoms", $_POST['prenoms']);
            $updateInfo->bindvalue(":telephone", $_POST['telephone']);
            $updateInfo->bindvalue(":whatsapp", $_POST['whatsapp']);
            $updateInfo->bindvalue(":description", $_POST['description']);
            $updateInfo->bindvalue(":email", $_SESSION['email']);
            $updateInfo->execute();

            // Mise a jour de toutes les annonces de la plateforme

            $updateAllAnnonces = $database->prepare("UPDATE annonces SET prenoms=:prenoms, tel=:tel WHERE email=:email");
...
<input type="text" class="form-control" value="<?=$dataUser['nom']?>" placeholder="Nom" name="nom">
</div>
</div>
<div class="col-md-6">
<div class="form-group">
<label>Prénoms <span class="text-danger">*</span></label>
<input type="text" class="form-control" value="<?=$dataUser['prenoms']?>" placeholder="Prénoms" name="prenoms">
</div>
</div>
<div class="col-md-6">
<div class="form-group">
...
<div class="col-md-6">
<div class="form-group">
<label>Téléphone <span class="text-danger">*</span> (avec l'indicatif du pays)</label>
<input type="text" class="form-control" value="<?=$dataUser['telephone']?>" placeholder="Phone" name="telephone">
</div>
</div>
<div class="col-md-12">
<div class="form-group">
<label>Description <span class="text-danger">*</span></label>
<textarea name="description" id="" cols="30" rows="10" class="form-control" placeholder="Entrer votre description ici"><?=$dataUser['description']?></textarea>
</div>
</div>
</div>
