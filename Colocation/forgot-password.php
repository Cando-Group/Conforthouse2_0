<?php
    ...
    if (isset($_POST['reset'])){
        ...
        if (!empty($_POST['email'])){
            ...
            $reqExistEmail = $database->prepare("SELECT * FROM collocusers WHERE email=:email");
            ...
            if ($countExistEmail != 1){
                ...
                $updateEmail = $database->prepare("UPDATE collocusers SET mdp = :mdp WHERE email = :email");
                $updateEmail->bindvalue(":mdp", sha1($token));
                        ...
                        <html>
                            <body>
                                ...
                                <div align="center" style="background: black;padding: 9px;">
                                        <img src="">
                                    <div style="color:#fff;font-style:italic;">Chers '.$nom.' vous avez perdu votre mot de passe. Nous vous fournissons un nouveau mot de passe par défaut. <br>Nouveau mot de passe : <br>Cliquez ici pour vous connecter <a href="affectlac.com/Colocation/login.php">Me connecter</a>
...
<button type="submit" name="reset" class="theme-btn"><i class="far fa-key"></i> Obtenir unnouveau mot de passe</button>
</div>
</form>
</div>
</div>
