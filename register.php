...
<?php
    ...
    if (isset($_POST['register'])){
        ...
        if (!empty($_POST['nom']) && !empty($_POST['prenoms']) && !empty($_POST['pays']) && !empty($_POST['ville']) && !empty($_POST['sexe']) && !empty($_POST['telephone']) && !empty($_POST['email']) && !empty($_POST['password']) && !empty($_POST['cpassword']) && !empty($_POST['whatsapp'])){

            $nom = htmlspecialchars($_POST['nom'], ENT_QUOTES, 'UTF-8');
            $prenoms = htmlspecialchars($_POST['prenoms'], ENT_QUOTES, 'UTF-8');
            ...
            if ($_POST['password'] === $_POST['cpassword']){
                ...
                $reqExistUser = $database->prepare("SELECT * FROM demarcheurs WHERE email=:email OR telephone=:telephone");
                $reqExistUser->bindvalue(":email", $email);
                $reqExistUser->bindvalue(":telephone", $telephone);
                ...
                if ($countExistUser){
                    ...
                    $token = token_random_string(55);

                    $confirmationLink = 'https://affectlac.com/verification_token.php?email=' . $email . '&token=' . $token . '&t=ai';
                    ...
                    <body>
                        <div align="center" style="background: black; padding: 9px;">
                            ...
                            <div style="color: #fff; font-style: italic;">
                                Chers ' . $nom . ' ' . $prenoms . ',<br>
                                Bienvenue sur Conforthouse. Pour activer votre compte, veuillez cliquer sur le lien de confirmation ci-dessous :
                                <br>
                    ...
                    $mail->SMTPDebug = 1;
                    $mail->SMTPSecure = 'tls';
                    $mail->Username = 'contact@affectlac.com';
                    $mail->Password = 'Nouvelle@version2.0';
                    $mail->setFrom('contact@affectlac.com', 'Conforth0use');
                    $mail->addReplyTo('contact@affectlac.com', "Conforth0use");
                    $mail->addAddress($email, $nom. $prenoms);
                    $mail->Subject = 'Confirmation de votre adresse mail';
                    
                    // Utilisez le message formaté
                    $mail->isHTML(true);
                    $mail->CharSet = 'UTF-8';
                    $mail->Body = $messages;
                    ...
                    if ($mail->send()){
                        ?>
                        <script>
                            swal("Succès", "Un mail vous a été envoyé", "success")
                        </script>
                        <?php

                        $insertUser = $database->prepare("INSERT INTO demarcheurs(nom, prenoms, pays, ville, sexe, telephone, whatsapp, email, mdp, token) VALUES(:nom, :prenoms, :pays, :ville, :sexe, :telephone, :whatsapp, :email, :mdp, :token)");
                        $insertUser->bindvalue(":nom", $nom);
                        $insertUser->bindvalue(":prenoms", $prenoms);
                        $insertUser->bindvalue(":pays", $pays);
                        $insertUser->bindvalue(":ville", $ville);
                        $insertUser->bindvalue(":sexe", $sexe);
                        $insertUser->bindvalue(":telephone", $telephone);
                        $insertUser->bindvalue(":whatsapp", $whatsapp);
                        $insertUser->bindvalue(":email", $email);
                        $insertUser->bindvalue(":mdp", password_hash($password, PASSWORD_BCRYPT));
                        $insertUser->bindvalue(":token", $token);
                        $insertUser->execute();

                    }else{
                        ?>
                        <script>
                            swal("Oups", "Une erreur est survenue lors de l'envoi du mail", "error")
                        </script>
                        <?php
                    }
...
<div class="container">
<h2 class="breadcrumb-title">Inscription</h2>
...
<form  method="post">
    <div class="form-group">
        ...
        <input type="text" id="nom" class="form-control" name="nom" placeholder="Votre nom">
        <i class="far fa-user"></i>
    </div>

    <div class="form-group">
        <label for="prenoms">Prénoms <span class="text-danger">*</span></label>
        <input type="text" id="prenoms" class="form-control" name="prenoms" placeholder="Vos prénoms">
        <i class="far fa-user"></i>
    </div>
    ...
    <div class="form-group">
        ...
        <input type="text" id="ville" class="form-control" name="ville" placeholder="Votre ville">
        <i class="far fa-user"></i>
    </div>
    ...
    <div class="form-group">
        <label for="telephone">Numéro de téléphone (avec indicatif du pays)<span class="text-danger">*</span></label>
        <input type="text" id="telephone" class="form-control" name="telephone" placeholder="Votre numéro de téléphone">
        <i class="far fa-user"></i>
    </div>

    <div class="form-group">
        <label for="whatsapp">Numéro Whatsapp (avec indicatif du pays)<span class="text-danger">*</span></label>
        <input type="text" id="whatsapp" class="form-control" name="whatsapp" placeholder="Votre numéro whatsapp">
        <i class="far fa-user"></i>
    </div>
