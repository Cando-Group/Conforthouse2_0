<?php
    ...
    $reqAgentAppartements = $database->prepare("SELECT * FROM demarcheurs WHERE email=:email");
    $reqAgentAppartements->bindvalue(":email", $email);
    $reqAgentAppartements->execute();

    $dataAgentAppartements = $reqAgentAppartements->fetch();
...
<div class="container">
<h2 class="breadcrumb-title">Appartements (<?=$dataAgentAppartements['nom']. " ". $dataAgentAppartements['prenoms']?>)</h2>
<ul class="breadcrumb-menu">
<li><a href="index.php">Accueil</a></li>
<li class="active">Appartements (<?=$dataAgentAppartements['nom']. " ". $dataAgentAppartements['prenoms']?>)</li>
</ul>
</div>
</div>
...
$reqEtudiantes = $database->prepare("SELECT * FROM annonces WHERE categorie=:categorie AND statut = :statut AND louer = :louer AND email=:email ORDER BY id DESC LIMIT :offset, :elementsParPage ");
$reqEtudiantes->bindValue(":statut", 1);
$reqEtudiantes->bindValue(":categorie", "Appartement");
$reqEtudiantes->bindValue(":louer", "non");
$reqEtudiantes->bindvalue(":email", $email);
...
if ($countEtudiantes > 0) { // Vérifier que la requête a réussi et qu'il y a des résultats
  ...
  <div class="col-md-6 col-lg-4 ">
        ...
        <div class="listing-bottom">
          <div class="listing-author">
            <div class="listing-author-img">
              <img src="upload/profile/<?=$dataAgentAppartements['photo']?>" alt="Profile" style="width:45px;height:45px;">
            </div>
    ...
    echo "<span class='theme-btn'><span class='theme-btn'>Aucune chambre trouvée.</span></span>";
}
...
<!-- Le reste de votre code HTML -->
        ...
        <div class="pagination-area mb-5">
            <div aria-label="Page navigation example">
                <ul class="pagination">
                <li class="page-item">
                    <a class="page-link" href="agentAppartements.php?email=<?=$email?>&page=<?php echo $pageActuelle - 1; ?>" aria-label="Previous">
                    <span aria-hidden="true"><i class="far fa-angle-double-left"></i></span>
                    </a>
                </li>
                <!-- Vous pouvez laisser les liens statiques pour les pages 1, 2 et 3 pour l'instant -->
                <li class="page-item <?php echo ($pageActuelle == 1) ? 'active' : ''; ?>"><a class="page-link" href="agentAppartements.php?email=<?=$email?>&page=1">1</a></li>
                <li class="page-item <?php echo ($pageActuelle == 2) ? 'active' : ''; ?>"><a class="page-link" href="agentAppartements.php?email=<?=$email?>&page=2">2</a></li>
                <li class="page-item <?php echo ($pageActuelle == 3) ? 'active' : ''; ?>"><a class="page-link" href="agentAppartements.php?email=<?=$email?>&page=3">3</a></li>
                <li class="page-item">
                    <a class="page-link" href="agentAppartements.php?email=<?=$email?>&page=<?php echo $pageActuelle + 1; ?>" aria-label="Next">
